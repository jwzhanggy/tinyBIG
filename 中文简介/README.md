...
长期以来，Function Learning 仅被视为连续函数拟合和回归任务的近似。其实在心理学和认知科学中，研究者们也使用函数学习概念来模拟人类和其他智能主体的刺激-反应关系的心理诱导过程，涉及知识获取、信息处理和推理。
本文认为，Function Learning 是智能模型学习中最基本的任务，涵盖了多种学习任务，例如：
...
受泰勒定理启发，RPN 将输入数据与模型参数分离，并将目标函数近似为数据扩展函数与参数和解函数的内积，然后加上余项函数。

## “广度”和“深度” RPN 模型结构

![architecture.png](architecture.png)

如上图所示，RPN提供了多种模型构建机制，允许用户根据具体的学习任务，来构造“广度模型结构”和“深度模型结构”。
...
* “拓展”和“嵌套”数据扩展函数：除此之外，RPN也提供了通过使用“拓展”和“嵌套”数据扩展函数，来更加便捷地构建“广度”和“深度”模型结构。具体如下图所示。
...
RPN实现了丰富的功能函数，具体列表如上图所示。

通过组合使用上述功能函数，RPN可以构建功能强大的模型结构，并且可以统一现有基础模型的表示，包括 PGMs, kernel SVM, MLP 和最近的 KAN。
...
`tinybig` 提供了一个丰富的预实现函数库，包括25类数据扩展函数、10类参数和解函数和5类余项函数，以及完整的模型框架和优化的模型训练管道。
...
在本文中，我们介绍了和解多项式网络（RPN）作为一种用于深度函数学习任务的新型基础模型。通过引入多层、多头和多通道，RPN具备灵活的模型架构，
