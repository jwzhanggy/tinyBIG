...
from tinybig.config.layer_configs import perceptron_layer_configs
...
class mlp_model_config(model_config):
    def __init__(
        self,
        dims: list[int] | tuple[int],
        name: str = 'mlp_model_config',
        ...
        *args, **kwargs
        ...
        if dims is None:
            raise ValueError("dimension list cannot be None...")
        layer_configs = [
            perceptron_layer_configs(
                ...
                d=d, r=r,
                *args, **kwargs
            ).get_configs() for m, n in zip(dims, dims[1:])
        ]
        parameters = {
            'name': name,
            'depth': len(layer_configs),
            'depth_alloc': [1]*len(layer_configs),
            'layer_configs': layer_configs,
            'device': device,
        }
        super().__init__(parameters=parameters, device=device, *args, **kwargs)
